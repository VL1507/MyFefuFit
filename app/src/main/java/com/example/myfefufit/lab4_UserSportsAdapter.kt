...
class lab4_UserSportsAdapter(
    private val items: List<UsersSportsItem>,
    ...
    override fun getItemViewType(position: Int): Int {
        return when (items[position]) {
            is UsersSportsItem.Header -> TYPE_HEADER
            is UsersSportsItem.Activity -> TYPE_ITEM
        }
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            TYPE_HEADER -> HeaderViewHolder(
                ...
                LayoutInflater.from(parent.context)
                    .inflate(R.layout.lab4_list_item_users_sports, parent, false)
            )
        }
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (val item = items[position]) {
            is UsersSportsItem.Header -> (holder as HeaderViewHolder).bind(item)
            is UsersSportsItem.Activity -> (holder as ActivityViewHolder).bind(item, onItemClick)
        }
    }
    ...
    class HeaderViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(item: UsersSportsItem.Header) {
    ...
    class ActivityViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(item: UsersSportsItem.Activity, onItemClick: (Int) -> Unit) {
